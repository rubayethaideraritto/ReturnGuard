export interface Order {
    ...
    // Customer data
    customer_id?: string;
    customer_return_history?: number; // Return rate percentage (0-100)
    customer_avg_order_value?: number; // Average order value for price comparison
    return_history_count?: number; // Number of past returns
    past_orders?: number;
    past_returns?: number;
    return_rate?: number; // Calculated: past_returns / past_orders
    account_age_days?: number;

    // Product data
    product_id?: string;
    product_return_rate?: number; // Historical return rate for this product
    avg_return_cost?: number;
    category_risk_score?: number; // 0-100
}

export interface RiskAnalysis {
    risk_score_percent: number;
    risk_label: 'MODERATE' | 'MEDIUM' | 'HIGH';
    confidence_percent: number; // 0-100 (changed from LOW/MED/HIGH)
    reasons: string[];
    reasoning_factors: string[]; // Detailed breakdown for UI
    estimated_savings: number;   // Potential money saved WITH AI
    prevention_chance: number;   // Probability of prevention
    potential_loss_without_ai: number; // What merchant loses if no intervention
    recommended_action: 'NO_ACTION' | 'CONFIRMATION' | 'INCENTIVE' | 'EXCHANGE' | 'REFUND';
    ...
    roi_breakdown: {
        ...
        prevention_prob: number;
    };
    comparison_insight: string; // "Killer comparison" line
}

export class OrderRiskAgent {
    private static readonly HANDLING_COST = 15.0;
    private static readonly SHIPPING_COST = 10.0;

    public analyzeOrder(order: Order): RiskAnalysis {
        let score = 0;
        ...
        let label: 'MODERATE' | 'MEDIUM' | 'HIGH' = 'MODERATE';
        ...
        let action: 'NO_ACTION' | 'CONFIRMATION' | 'INCENTIVE' | 'EXCHANGE' | 'REFUND' = 'NO_ACTION';
        let actionDesc = '';
        if (label === 'HIGH') {
            action = 'CONFIRMATION';

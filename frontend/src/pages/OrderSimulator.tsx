...
export const OrderSimulator = () => {
    ...
    const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
        ...
        try {
            if (!user) throw new Error("You must be logged in to run simulation results");
            setError(null);
    ...
    const handleShopifySync = async () => {
        ...
        try {
            if (!user) throw new Error("You must be logged in to sync with Shopify");
            const token = await user.getIdToken();
            const apiUrl = import.meta.env.VITE_API_URL || 'http://localhost:3000';

            const res = await fetch(`${apiUrl}/api/shopify/analyze/${shopifyOrderId}`, {
                method: 'GET',
                headers: {
                    'Authorization': `Bearer ${token}`
                }
                ...
                <Card className="lg:col-span-4 border-slate-100 shadow-sm rounded-[2.5rem] bg-white overflow-hidden">
                    <form onSubmit={handleSubmit}>
                        ...
                        <CardContent className="p-8 space-y-8">
                            {/* Shopify Connectivity Alert/Toggle */}
                            <div className="p-5 rounded-3xl bg-indigo-50/50 border border-indigo-100/50 space-y-4">
                                <div className="flex items-center justify-between">
                                    <h4 className="text-[10px] font-bold text-indigo-900/40 uppercase tracking-widest">Shopify Live Sync</h4>
                                    <div className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
                                </div>
                                <div className="flex gap-2">
                                    <Input
                                        placeholder="Order ID (e.g. 56228383321)"
                                        value={shopifyOrderId}
                                        onChange={(e) => setShopifyOrderId(e.target.value)}
                                        className="h-10 rounded-[1rem] border-white bg-white text-[10px] font-bold shadow-sm"
                                    />
                                    <Button
                                        type="button"
                                        onClick={handleShopifySync}
                                        disabled={syncing || !shopifyOrderId}
                                        className="h-10 px-4 rounded-[1rem] bg-indigo-600 hover:bg-indigo-700 text-white shadow-lg active:scale-95 transition-all"
                                    >
                                        {syncing ? <RefreshCw className="w-3.5 h-3.5 animate-spin" /> : <Zap className="w-3.5 h-3.5 fill-current" />}
                                    </Button>
                                </div>
                                <p className="text-[9px] text-indigo-900/40 font-bold leading-tight uppercase tracking-wider">
                                    Import real-time risk markers directly from your store.
                                </p>
                            </div>
                            ...
                            <div className="relative">
                                ...
                                <div className="relative flex justify-center text-[10px] uppercase tracking-[0.2em] font-bold">
                                    <span className="bg-white px-3 text-slate-300">Or Manual Test</span>
                                </div>
                            </div>
                            ...
                            <div className="space-y-6">
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        ...
                                        <Input name="return_history_count" type="number" defaultValue="1" className="h-12 rounded-2xl border-slate-100 bg-slate-50/50 text-xs font-bold focus:bg-white transition-all" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Total Orders</label>
                                        <Input name="past_orders" type="number" defaultValue="5" className="h-12 rounded-2xl border-slate-100 bg-slate-50/50 text-xs font-bold focus:bg-white transition-all" />
                                    </div>
                                </div>
                                ...
                                <div className="grid grid-cols-2 gap-4">
                                    <div className="space-y-1.5">
                                        ...
                                        <Input name="account_age_days" type="number" defaultValue="63" className="h-12 rounded-2xl border-slate-100 bg-slate-50/50 text-xs font-bold focus:bg-white transition-all" />
                                    </div>
                                    <div className="space-y-1.5">
                                        <label className="text-xs font-bold text-slate-400 uppercase tracking-widest ml-1">Account Type</label>
                                        <select name="is_guest" className="h-12 w-full rounded-2xl border border-slate-100 bg-slate-50/50 px-4 text-xs font-bold focus:outline-none focus:ring-4 focus:ring-indigo-500/5 focus:border-indigo-500 transition-all appearance-none">

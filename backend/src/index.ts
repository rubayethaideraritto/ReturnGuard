...
import { verifyShopifyWebhook } from './middleware/verifyWebhook';
...
app.use('/api/webhooks', verifyShopifyWebhook, webhookRouter);
...
// ---- Shopify Sync & Analyze ----
app.get('/api/shopify/analyze/:orderId', async (req: Request, res: Response) => {
    const { orderId } = req.params;
    const userId = (req as any).userId;

    try {
        console.log(`Analyzing Shopify Order: ${orderId}`);
        const orderData = await shopifyService.getOrderAnalysisData(orderId);

        const riskAnalysis = riskAgent.analyzeOrder(orderData);

        // Generate AI-like narrative
        const narrative = narrativeEngine.generateRiskNarrative({ order: orderData, riskAnalysis });
        const comparisonInsight = narrativeEngine.generateComparisonInsight({ order: orderData, riskAnalysis });
        const customerMessage = narrativeEngine.generateCustomerMessage({ order: orderData, riskAnalysis }, 'WhatsApp');
        ...
        console.error('Shopify Analysis Error:', error.message);
        res.status(500).json({ error: error.message || 'Failed to analyze Shopify order' });
    }
});
...
app.use((req, res) => {
    ...
    res.status(404).json({
        ...
        path: req.url,
        method: req.method,
        suggestion: "Check if your VITE_API_URL includes /api or has a trailing slash"
    });
});
